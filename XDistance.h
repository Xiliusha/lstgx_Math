...
namespace xmath
{
	namespace distance
	{
		...
		inline float Point_OBB_squared(const Vec2& p0, const Vec2& p1, float halfW, float halfH, float aCos, float aSin)
		{
			const auto d = p0 - p1;
		...
		inline float Point_OBB(const Vec2& p0, const Vec2& p1, float halfW, float halfH, float aCos, float aSin)
		{
			return std::sqrt(Point_OBB_squared(p0, p1, halfW, halfH, aCos, aSin));
		}
		// faster when point not in triangle
		float Point_Triangle(const Vec2& p, const Vec2& A,const Vec2& B,const Vec2& C);
		// faster when point in triangle
		// reference: https://www.geometrictools.com/Documentation/DistancePoint3Triangle3.pdf
		float Point_Triangle2(const Vec2& p, const Vec2& A,const Vec2& B,const Vec2& C);

		float Point_Diamond(const Vec2& p0, const Vec2& p1, float halfW, float halfH, float rotation);

		float Point_Parallelogram(const Vec2& p0, const Vec2& p1, const Vec2& halfDiagA, const Vec2& halfDiagB);
		// accurate, slow
		float Point_Ellipse(const Vec2& p0, const Vec2& p1, float a, float b, float rotation);
		// approximate, 7x faster, about 10ppm error
		float Point_Ellipse2(const Vec2& p0, const Vec2& p1, float a, float b, float rotation);
	}
}
